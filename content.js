...
function updateButtonStates() {
    ...
    buttons.progressDisplay.innerHTML = `已采集: ${posts.length} 帖子, ${totalComments} 评论`;
...
function collectPosts() {
    ...
    try {
        ...
        if (window.location.pathname.includes('/explore/')) {
            ...
            // 如果是搜索结果页面
            let newPosts = document.querySelectorAll('.note-item, [data-v-note-item], .search-result-item');
            ...
            newPosts.forEach(post => {
                try {
                    ...
                    if (!posts.some(p => p.link === link)) {
                        const postData = {
                            title: post.querySelector('.title, .content, .desc')?.textContent.trim() || '',
                            author: post.querySelector('.user-nickname, .author, .user-info-name')?.textContent.trim() || '',
                            likes: parseLikes(post.querySelector('.like-wrapper .count, .likes, .interaction-info span')?.textContent.trim() || '0'),
                            link: link,
...
async function collectPostDetails() {
    const postData = {
        ...
        author: document.querySelector('.user-name')?.textContent.trim() || '',
        editTime: document.querySelector('.date')?.textContent.trim() || '',
        ...
        commentCount: document.querySelector('.comment-wrapper .count')?.textContent.trim() || '0',
        link: window.location.href,
        comments: []
    };

    // 解析数字
    postData.likes = parseLikes(postData.likes);
    postData.collects = parseLikes(postData.collects);
    postData.commentCount = parseLikes(postData.commentCount);
    ...
    if (!posts.some(p => p.link === postData.link)) {
        ...
        // 采集评论
        await collectComments(postData);
    } else {
        console.log('该帖子已存在，跳过采集');
    }
}

async function collectComments(postData) {
    let hasNextPage = true;
    let pageCount = 0;
    while (hasNextPage && isCollecting && !isPaused) {
        await new Promise(resolve => setTimeout(resolve, 2000)); // 等待页面加载
        const commentElements = document.querySelectorAll('.comment-item');
        console.log(`正在采集第 ${pageCount + 1} 页评论，找到 ${commentElements.length} 条评论`);
        
        for (let commentEl of commentElements) {
            const comment = {
                content: commentEl.querySelector('.content')?.textContent.trim() || '',
                time: commentEl.querySelector('.time')?.textContent.trim() || '',
                author: commentEl.querySelector('.user-nickname')?.textContent.trim() || ''
            };
            if (!postData.comments.some(c => c.content === comment.content && c.author === comment.author)) {
        ...
        // 检查是否有下一页
        const nextPageButton = document.querySelector('.next-page-button:not([disabled])');
        if (nextPageButton) {
            nextPageButton.click();
            pageCount++;
            console.log(`点击下一页，等待加载第 ${pageCount + 1} 页评论`);
        } else {
            hasNextPage = false;
            console.log('没有更多评论页面，结束采集');
        }

        // 添加安全检查，防止无限循环
        if (pageCount > 100) {
            console.log('达到最大页数限制，停止采集评论');
            break;
        }
    }
    console.log(`评论采集完成，共采集 ${postData.comments.length} 条评论`);
}

function parseLikes(likes) {
    if (typeof likes === 'string') {
        likes = likes.toLowerCase().trim();
        if (likes.endsWith('w') || likes.endsWith('万')) {
            return Math.round(parseFloat(likes.replace(/[w万]/g, '')) * 10000).toString();
...
function updateFloatingButton() {
    if (isCollecting) {
        ...
        let totalLikes = posts.reduce((sum, post) => sum + parseInt(post.likes || 0), 0);
        let totalComments = posts.reduce((sum, post) => sum + parseInt(post.commentCount || 0), 0);
        
        buttons.progressDisplay.innerHTML = `已采集: ${posts.length} 帖子, ${totalComments} 评论, ${totalLikes} 点赞`;
